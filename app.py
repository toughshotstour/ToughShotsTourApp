...
from lane_scoring import (
    ...
    publish_manifest, save_manifest,
)
...
class ToughShotsApp(tk.Tk):
    ...
    def _build_lanes_page(self):
        page = WorkflowPage(
            ...
            "Randomly balance all bowlers across the available lanes, publish lane-specific QR scoring pages to your cloud server, and create one six-game printable score sheet per lane.",
        )
        body = page.body
        ...
        ttk.Button(actions, text="Retry Publish", command=self.retry_lane_publish, style="Secondary.TButton").pack(side="left", padx=8)
        ttk.Button(actions, text="Sync Mobile Scores", command=self.sync_mobile_scores, style="Secondary.TButton").pack(side="left")
        ...
        ttk.Label(
            ...
            text=(
                ...
                "writes lane_assignments.csv, and generates a printable PDF with one QR-coded sheet per lane. Re-running it creates a new draw and resets cloud qualifying scores for this tournament session."
            ),
            style="CardText.TLabel",
            wraplength=760,
            justify="left",
