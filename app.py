...
class ToughShotsApp(tk.Tk):
    ...
    def _build_results_page(self):
        page = WorkflowPage(
            ...
            "Keep the private permanent bowler database in sync from the demographic form, manage Jr. Gold status, and publish public standings/results to the Render site.",
        )
        body = page.body
        FilePicker(
            body, label="Demographic form CSV", variable=self.demographics_var,
            choose_command=lambda: self.choose_csv(self.demographics_var),
            hint="Used to create/update permanent bowlers. The form must include first name, last name, gender, birthdate, and a column containing USBC in its heading.",
        ).grid(row=0, column=0, sticky="ew", pady=(0, 14))

        settings = ttk.Frame(body, style="Card.TFrame")
        settings.grid(row=1, column=0, sticky="ew", pady=(0, 14)); settings.columnconfigure(1, weight=1)
        ...
        ttk.Label(settings, text="Cloud URL and admin key are shared with Lanes + Mobile.", style="Hint.TLabel").grid(row=1,column=0,columnspan=2,sticky="w",pady=(4,0))

        ttk.Label(body, text="Private bowler database", style="Section.TLabel").grid(row=2,column=0,sticky="w",pady=(4,8))
        row1=ttk.Frame(body,style="Card.TFrame"); row1.grid(row=3,column=0,sticky="w",pady=(0,14))
        ttk.Button(row1,text="Import / Update from Demographic Form",command=self.import_permanent_bowlers,style="Primary.TButton").pack(side="left")
        ...
        ttk.Label(body, text="Public Render site", style="Section.TLabel").grid(row=4,column=0,sticky="w",pady=(4,8))
        row2=ttk.Frame(body,style="Card.TFrame"); row2.grid(row=5,column=0,sticky="w",pady=(0,12))
        ...
        ttk.Label(body,text=(
            "The permanent database is not exposed on the public website. Bowler IDs are derived from the USBC ID by padding zeros on the right to 10 digits; when that ID is already used by a different bowler, the final digit is advanced to make the new ID unique. "
            "Jr. Gold status is preserved across later demographic imports. BOY points are calculated automatically from qualifying place, match wins, and champion/runner-up bonuses. Jr. Gold standings use the separate merge and cut settings saved in Tournament Manager."
        ),style="CardText.TLabel",wraplength=760,justify="left").grid(row=6,column=0,sticky="w",pady=(10,0))
        return page
    ...
    def import_permanent_bowlers(self):
        ...
        if not demo.is_file():
            messagebox.showerror("Missing demographic form","Choose the demographic form CSV first.",parent=self); return
        ...
        if not self._archive_inputs("permanent_bowler_import",("demographic_form",demo)): return
        self.status_var.set("Updating permanent bowler database…")
        def worker():
            try:
