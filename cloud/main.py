...
async def api_archive_tournament(request: Request, x_admin_key: str | None = Header(default=None)):
    ...
    with db() as conn:
        ...
        conn.commit()
    return {"ok": True, "archived": len(rows), "unmatched_bowlers": unmatched, "points_formula_configured": True}
...
def public_home():
    ...
    live_text = f"Latest: {html.escape(live['name'])}" if live else "Ready for the next published tournament"
    body = f"<h2>Welcome</h2><p class='muted'>{live_text}</p><div class='grid'><a class='tile' href='/standings'><h2>Qualifying Standings</h2><p>Live qualifying results for all seven divisions.</p></a><a class='tile' href='/bowler-of-the-year'><h2>Bowler of the Year</h2><p>Season points and performance for all seven divisions.</p></a><a class='tile' href='/jr-gold'><h2>Jr. Gold Qualifying</h2><p>Current Jr. Gold qualifier standings and cut lines.</p></a><a class='tile' href='/archive'><h2>Tournament Archive</h2><p>Search past tournaments and individual bowler performances. {archive_count} tournament(s) archived.</p></a></div>"
    return _page("Tough Shots", body)


@app.get("/standings", response_class=HTMLResponse)
def standings_index():
    tiles = "".join(f"<a class='tile' href='/standings/{_division_slug(d)}'><h2>{html.escape(d)}</h2><p>View current qualifying standings</p></a>" for d in DIVISIONS)
...
def boy_index():
    tiles = "".join(f"<a class='tile' href='/bowler-of-the-year/{_division_slug(d)}'><h2>{html.escape(d)}</h2><p>Season performance standings</p></a>" for d in DIVISIONS)
    return _page("Bowler of the Year", f"<p><a href='/'>← Home</a></p><h2>Bowler of the Year</h2><p class='muted'>Points: qualifying place (last=1 through first), plus 5 per match win, plus 10 runner-up / 20 champion bonus.</p><div class='grid'>{tiles}</div>")
...
def boy_division(division_slug: str):
    ...
    return _page(f"{division} Bowler of the Year", f"<p><a href='/bowler-of-the-year'>← Divisions</a></p><h2>{html.escape(division)} — Bowler of the Year</h2><p class='muted'>Qualifying points increase from 1 for last place through the winner, plus 5 per match win and finish bonuses.</p>{table}")
...
def jr_gold_index():
    ...
    tiles="".join(f"<a class='tile' href='/jr-gold/{_jg_slug(g['group_name'])}'><h2>{html.escape(g['group_name'])}</h2><p>View Jr. Gold qualifier standings</p></a>" for g in groups)
    detail=(f"<p><strong>{html.escape(t['name'])}</strong><br><span class='muted'>{html.escape(t['event_date'])}</span></p>" if t else "<p class='muted'>No Jr. Gold standings have been published yet.</p>")
    return _page("Jr. Gold Qualifying",f"<p><a href='/'>← Home</a></p><h2>Jr. Gold Qualifying</h2><p class='muted'>Includes bowlers marked JG or Q in the private permanent-bowler database.</p>{detail}<div class='grid'>{tiles}</div>")
...
def jr_gold_group(group_slug: str):
    ...
    return _page(f"{g['group_name']} Jr. Gold",f"<p><a href='/jr-gold'>← Jr. Gold Divisions</a></p><h2>{html.escape(g['group_name'])} — Jr. Gold Qualifying</h2><p><strong>{html.escape(t['name'])}</strong><br><span class='muted'>{html.escape(t['event_date'])} · {html.escape(note)}</span></p>{table}")
