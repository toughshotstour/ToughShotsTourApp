...
Before scoring, use **Manage Scorer PINs** on the desktop Lanes + Mobile page to add each authorized scorer. The server generates a private six-digit PIN for that scorer. A scorer scans the lane-pair QR code with a phone camera, enters their PIN the first time, and then sees both lanes on that sheet with six score fields per bowler. Their phone remains signed in for up to 12 hours unless they sign out or their PIN is reset. Scorecards do not lock after submission; authorized scorers can make corrections later.
