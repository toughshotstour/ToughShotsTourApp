...
def create_scorer(base_url, admin_key, name):
    base_url = normalize_base_url(base_url)
    name = (name or "").strip()
    if not name:
        raise ValueError("Enter a scorer name.")
    return _json_request(
        f"{base_url}/api/scorers", method="POST", payload={"name": name}, admin_key=admin_key.strip()
    )


def reset_scorer_pin(base_url, admin_key, scorer_id):
    base_url = normalize_base_url(base_url)
    return _json_request(
        f"{base_url}/api/scorers/{int(scorer_id)}/reset-pin", method="POST", payload={}, admin_key=admin_key.strip()
    )
